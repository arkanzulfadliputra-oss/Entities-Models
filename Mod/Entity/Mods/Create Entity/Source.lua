...
local function PlayJumpscareAndKill(jumpscareConfig)
    ...
    if jumpscareConfig and type(jumpscareConfig) == "table" then
...
local function PlayDebugSound(soundConfig)
    if not soundConfig or soundConfig.SoundID == "rbxassetid://0" then return end
    ...
    for effectName, settings in pairs(soundConfig.SoundEffect or {}) do
...
local function DamagePlayer(entity)
    ...
    if newHealth == 0 then
        if config.Jumpscare and config.Jumpscare.Enabled then
            PlayJumpscareAndKill(config.Jumpscare.Config)
        else
            if Humanoid then
                Humanoid.Health = 0
            end
...
Module.Create = function(self, config)
    ...
    return {
        ...
        OtherCallback = function(self, callback)
            if typeof(callback) == "function" then
...
Module.Run = function(self, entity, copyEntity)
    ...
    if config.Debugging and config.Debugging.Spawned then
        PlayDebugSound(config.Debugging.Spawned.Sound)
        if typeof(config.Debugging.Spawned.Callback) == "function" then
            task.spawn(config.Debugging.Spawned.Callback, entity)
        end
    end
    ...
    task.spawn(function()
        ...
        if config.Debugging and config.Debugging.Despawn then
            PlayDebugSound(config.Debugging.Despawn.Sound)
            if typeof(config.Debugging.Despawn.Callback) == "function" then
                task.spawn(config.Debugging.Despawn.Callback, entity)
            end
        end
